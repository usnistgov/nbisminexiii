...
if ( nminutiae > max_minutiae ) {
	if ( verbose_load )
		fprintf( errorfp, "%s: WARNING: bz_load(): trimming minutiae to the %d of highest quality\n",
...
if ( s == XYT_NULL ) {
	fprintf( errorfp, "%s: ERROR: malloc() failure while loading minutiae file \"%s\" failed: %s\n",
							get_progname(),
							xyt_file,
							strerror(errno)
							);
